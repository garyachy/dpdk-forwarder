...
RUN cd /build/dpdk && \
    meson setup build \
      ...
      -Ddisable_libs="bbdev,bitratestats,bpf,cfgfile,efd,fib,flow_classify,gpudev,gro,gso,ipsec,jobstats,latencystats,member,metrics,mldev,pdcp,pdump,power,rcu,rib,sched,security,vhost" \
...
COPY --from=dpdk-builder /usr/local/bin /usr/local/bin
